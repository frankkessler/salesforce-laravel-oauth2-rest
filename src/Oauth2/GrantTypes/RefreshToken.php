...
class RefreshToken{
     ...
     */
    public static function getAccessTokenWithRefreshToken($service_authorization_url, $config){
        $config = array_merge(self::getDefaultConfig(), $config);

        return $service_authorization_url.'?grant_type='.$config['grant_type'].'&client_id='.$config['client_id'].'&client_secret='.$config['client_secret'].'&refresh_token='.$config['refresh_token'];
    }
